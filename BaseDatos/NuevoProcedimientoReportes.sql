...
select		CONVERT(int ,row_number() OVER(Order By p.POST_Interno)) StructId, a.ALIM_Codigo FeederCode, a.ALIM_Etiqueta FeederLabel, p.POST_Interno ElementId, 'POST' ElementType, p.POST_CodigoNodo CodIns, p.POST_Etiqueta Label, 
			isnull(d.tipiCode,'') TypificationCode, '' SedType, '' StartNode, '' EndNode, isnull(f.Ruta,'') PhotosPath, isnull(d.DEFI_TipoMaterial,'') ElementMaterial,isnull(d.DEFI_TipoRetenida,'') HeldType, isnull(d.DEFI_NumSuministro,'') Supply,
					...
					isnull(d.DEFI_NumPostes,0) PostNum, isnull(f.Cantidad,0) PhotosQuantity
...
(
			select	(select c.CODI_Codigo
					...
					d.DEFI_DistHorizontal, d.DEFI_DistVertical, d.DEFI_ArmadoMaterial, d.DEFI_TipoArmado, d.DEFI_EstadoSubsanacion, d.DEFI_NumPostes, d.DEFI_Estado
			from	Deficiencias	d
...
select		CONVERT(int ,row_number() OVER(Order By v.VANO_Interno)) StructId,a.ALIM_Codigo FeederCode,a.ALIM_Etiqueta FeederLabel, v.VANO_Interno ElementId, 'VANO' ElementType, v.VANO_Codigo CodIns, v.VANO_Etiqueta Label, 
			Isnull(d.tipiCode,'') TypificationCode, '' SedType, isnull(d.DEFI_NodoInicial,'') StartNode, isnull(d.DEFI_NodoFinal,'') EndNode, isnull(f.Ruta,'') PhotosPath, isnull(d.DEFI_TipoMaterial,'') ElementMaterial,isnull(d.DEFI_TipoRetenida,'') HeldType, isnull(d.DEFI_NumSuministro,'') Supply,
					isnull(d.DEFI_DistHorizontal,0) DistHorizontal, isnull(d.DEFI_DistVertical,0) DistVertical, isnull(d.DEFI_ArmadoMaterial,'') ArmedMaterial, isnull(d.DEFI_TipoArmado,'') ArmedType, CONCAT(isnull(d.DEFI_Estado,''),isnull(d.DEFI_EstadoSubsanacion,'')) Subsanacion,
					isnull(d.DEFI_NumPostes,0) PostNum, isnull(f.Cantidad,0) PhotosQuantity
from		Vanos			v
