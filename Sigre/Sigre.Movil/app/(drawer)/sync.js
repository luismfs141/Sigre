...
export default function Sync() {
  ...
  const { user } = useContext(AuthContext);
  const { offlineLoading, downloadDatabase, syncing, syncAllPending } = useOffline();
  const { dbName, dbReady, openLocalDB, setDbName, selectedFeeder, setSelectedFeeder } = useDatos();
  ...
  // CARGAR SEDS LOCALES CUANDO EXISTE BASE
  //───────────────────────────────────────────────
  const triedOpenRef = useRef(false);

  useEffect(() => {
    // cada vez que cambie de dbName, permitimos reintentar apertura
    triedOpenRef.current = false;
  }, [dbName]);
  ...
  useEffect(() => {
    const loadLocalSeds = async () => {
      ...
      // Si el archivo existe pero la DB aún no está abierta → ábrela primero
      if (!dbReady) {
        if (!triedOpenRef.current) {
          triedOpenRef.current = true;
          console.log("🗄️ [Sync] DB no lista, intentando abrir:", dbName);
          await openLocalDB();
        } else {
          console.log("⏳ [Sync] Esperando dbReady... dbName:", dbName);
        }
        return;
      }

      try {
        console.log("✅ [Sync] DB lista, cargando SEDs locales:", dbName);
        const data = await fetchAllSedsLocal();
        setLocalSeds(Array.isArray(data) ? data : []);
      } catch (e) {
        console.log("❌ [Sync] Error cargando SEDs locales:", e);
        setLocalSeds([]);
      }
    };
  ...
  //───────────────────────────────────────────────
  // Helper nombre export
  //───────────────────────────────────────────────
  const pad2 = (n) => String(n).padStart(2, "0");

  const buildStamp = (d = new Date()) => {
  ...
  const handleDownload = async () => {
    if (dbExists) {
      return Alert.alert(
        ...
        "Primero elimina la base local para poder descargar una nueva."
      );
    }
  ...
  const handleExport = async () => {
    try {
      ...
      // ✅ 1) Nombre de usuario desde login
      const fullNameRaw = `${user?.nombre ?? ""} ${user?.apellido ?? ""}`.trim();
      const userSeg = safeName(fullNameRaw || "USUARIO");

      // ✅ 2) Timestamp AAAAMMDD_HHMMSS
      const stamp = buildStamp();

      // ✅ 3) Nombre final
      const exportName = `${stamp}-${userSeg}.db`;

      // ✅ 4) Copiar a cache con nombre nuevo (Sharing respeta este nombre)
      const exportUri = `${FileSystem.cacheDirectory}${exportName}`;

      // 🧾 LOGS para depurar
      console.log("📤 [EXPORT] dbName:", dbName);
      console.log("📤 [EXPORT] dbPath:", dbPath);
      console.log("📤 [EXPORT] user:", user);
      console.log("📤 [EXPORT] fullNameRaw:", fullNameRaw);
      console.log("📤 [EXPORT] userSeg:", userSeg);
      console.log("📤 [EXPORT] exportName:", exportName);
      console.log("📤 [EXPORT] exportUri:", exportUri);

      // ⚠️ si ya existe, bórralo para evitar conflicto
      const existing = await FileSystem.getInfoAsync(exportUri);
      if (existing.exists) {
        await FileSystem.deleteAsync(exportUri, { idempotent: true });
        console.log("🧹 [EXPORT] Se eliminó export anterior con mismo nombre");
      }

      await FileSystem.copyAsync({ from: dbPath, to: exportUri });

      const exportedInfo = await FileSystem.getInfoAsync(exportUri);
      console.log("✅ [EXPORT] Copiado OK? exists:", exportedInfo.exists, "size:", exportedInfo.size);
  ...
  const handleDelete = async () => {
    if (!dbExists) {
      return Alert.alert("Aviso", "No hay base local para eliminar.");
    }
        ...
        { text: "Cancelar", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              ...
              await AsyncStorage.removeItem("selectedFeeders");
              await AsyncStorage.removeItem("db_name"); // ✅ esta es la correcta
      ...
      {isLoading ? (
        ...
        <>
          ...
          <View style={styles.header}>
            ...
            <View style={styles.headerButtons}>
              <Button title="➕ Alimentador" onPress={() => setModalVisible(true)} disabled={dbExists} />
          ...
          <View style={styles.footer}>
            <View style={styles.footerRow}>
              <View style={styles.footerCol}>
                <Button
                  title={dbExists ? "📦 Base detectada" : "📥 Descargar Base"}
