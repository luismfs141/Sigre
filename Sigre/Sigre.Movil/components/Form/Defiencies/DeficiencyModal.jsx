...
//       const res = await saveDeficiency(localDef, userId);
...
//       // ✅ Mostrar el mismo mensaje profesional del pin
//       if (res?.pinMsg) {
//         Alert.alert("📌 Estado de inspección actualizado", res.pinMsg);
//       }
...
export default function DeficiencyModal({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // ✅ IMPORTANTÍSIMO: garantizamos que SIEMPRE viaje idElemento/tipoElemento
      const payload = {
        ...localDef,
      ...
      // ✅ Mostrar mensaje del PIN y recién cerrar con OK
      if (res?.pinMsg) {
...
const styles = StyleSheet.create({
  ...
  // (lo dejo igual que tu archivo: hay 2 "buttons"; RN toma el último)
  buttons: {
    flexDirection: "row",
    justifyContent: "space-between",
    marginTop: 12
